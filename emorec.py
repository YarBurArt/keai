...
def emotion(image, returndata=False):
    ...
    for x, y, w, h in faces:
        ...
        if predicted_emotion == 'Happy':
            imgshow.showimg_tk('graphics/r1.png', 
                               "it's good that you feel alive")
...
while True:
    while True:
        ...
        # src = cv2.cuda_GpuMat()
        # src.upload(frame)
        # clahe = cv2.cuda.createCLAHE(clipLimit=5.0, 
                                       tileGridSize=(8, 8))
        ...
        image_pre = cv2.flip(frame, 1)

        image = emotion(image_pre, returndata=True)
